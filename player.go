...
func (p *Player) ExecuteCommand(command string) bool {
	...
	verbMap := map[string]func([]string) bool{
		...
		"SHOOT":      func(args []string) bool { p.Println("You shoot into the air wildly."); return true },
	}

	delegated := false

	// DEBUG: Print what command we're trying to process
...
func (p *Player) Run() {
	...
	for {
		...
		fmt.Printf("[DEBUG] Raw input: %q\n", cmd)

		// Fix the string processing
		cmd = strings.TrimSpace(cmd) // Remove ALL whitespace
		cmd = strings.ToUpper(cmd)   // Convert to uppercase

		// DEBUG: Show processed input
		fmt.Printf("[DEBUG] Processed input: %q\n", cmd)

		// replace alias mapping
		cmd = p.VerbAliasReplace(cmd)

		// DEBUG: Show after alias replacement
		fmt.Printf("[DEBUG] After aliases: %q\n", cmd)
